...
## 🆕 [14-15 Sept 2026, sesi kesepuluh] Batch besar Android polish + bug fix, Master Rate (Angular), testing infra

**Konteks**: sesi maraton nge-cover laporan bug user pas nyoba app beneran di HP (bukan cuma compile-check), plus user mulai nulis unit/UI test sendiri (template dari referensi kelas yang beberapa kali gak cocok sama arsitektur project — dicatat detail di bawah biar gak keulang).

### Guest-access nav guard — akhirnya beneran ke-enforce
`MainScreen.kt` dari awal SAMA SEKALI gak ngecek status login pas tap bottom nav — guest bisa buka Riwayat/Ajukan/Notifikasi/Profil langsung padahal harusnya cuma Beranda yang boleh. Fix: `MainViewModel.kt` baru (`isLoggedIn: StateFlow<Boolean>` dari `TokenDataStore.tokenFlow`), dipasang di `AnimatedBottomNavBar`'s `onNavigate` — tab selain "home" tanpa login di-redirect ke Login, bukan langsung `navigate()`.

### Navbar floating + glass — 3 iterasi sampai kerasa "kaca"
1. Padding luar `AnimatedBottomNavBar.kt` ternyata `60.dp` di semua sisi (sisa placeholder gak pernah di-tuning) — Scaffold jadinya reservasi ruang kosong gede banget di bawah layar, bikin konten Simulasi kepotong prematur DAN area itu warnanya flat (bukan gradient layar), jadi efek glass gak nembus apa-apa. Dikecilin ke `24.dp` horizontal / `8.dp`-`16.dp` vertical + `navigationBarsPadding()`.
2. `MainScreen.kt` direstruktur: khusus route "home", `Scaffold`'s `innerPadding` bottom sengaja DIABAIKAN (cuma pakai top padding) biar background blob Beranda beneran nyambung ke tepi layar asli, nyediain "warna" buat di-blend efek glass navbar. Layar lain (Riwayat/Ajukan/dst) tetap dapet innerPadding penuh seperti biasa — gak diubah.
3. **Real backdrop-blur (`Modifier.blur()`) TERNYATA gak bakal kerja buat kasus ini** — di Compose itu ngeblur ISI layer itu sendiri (kayak blur foto), bukan "nembus" ngeblur apa yang di belakangnya (beda dari CSS `backdrop-filter`) — backdrop-blur beneran butuh library kayak Haze yang sengaja gak ditambahin (gak bisa validasi versi/API-nya tanpa akses internet real-time, resiko rusak build). Diganti pendekatan aman: `GlassBg` opacity diturunin `0.75`→`0.42` (user re-tuning sendiri belakangan jadi `0.60`, dibiarin — preferensi visual dia), plus `GlassSheen` (gradient highlight diagonal tipis, trik umum bikin permukaan flat kerasa kaca tanpa blur beneran).

### Blob background disebarin ke SEMUA layar (sebelumnya cuma Home)
`sakukuBlobBackground()` (`ui/theme/Gradients.kt`, dibikin sesi sebelumnya) tadinya cuma dipasang di `HomeScreen.kt`. Diterapin ke 9 layar sekaligus: Notifikasi, Riwayat, Ajukan, Profil, Cek Plafond, Login, Register, Onboarding, Welcome — semua ganti dari `SakukuPageGradient` flat.

### Bug navigasi nyata ketemu & fixed
1. **Auto-shutdown app pas buka tab Ajukan** — dilaporin user, udah ditelusuri `PengajuanViewModel`/`PengajuanRepository`/`CustomerRepository`/`PengajuanScreen.kt` lengkap, semua bersih secara kode. **Belum ketemu root cause** — kemungkinan gede itu efek dari kondisi build yang sempet gak sinkron (banyak file lain lagi error compile bersamaan, lihat bagian AuthRepository di bawah), tapi belum diverifikasi ulang abis semua fix lain masuk. Perlu Logcat kalau masih kejadian.
2. **Logout dead-end di Login, gak bisa balik ke Beranda tamu** — root cause ketemu: `AppNavigation.kt`'s `onLoggedOut` nge-clear SELURUH back stack (`popUpTo("splash", inclusive=true)`) terus taro "login" sendirian — tombol back di Login jadi gak ada tujuan (bukan literally "loop", tapi dead-end). **Fix**: abis logout sekarang balik ke `"main"` (Beranda sebagai tamu), sesuai guest-access model app ini — bukan ke `"login"` sama sekali.
3. **Login gagal parse, error kotlinx.serialization "fields[statusCode, message] are required...missing at path:$"** — root cause: `CustomerAuthController.login()` (backend) balikin `AuthResponseDTO` MENTAH (`{token,type}`), BUKAN dibungkus `ApiResponse<>` kayak kebanyakan endpoint lain, tapi `ApiService.kt` (Android) declare return type-nya `ApiResponse<LoginResponseData>`. **Fix**: ganti ke `LoginResponseData` polos, `AuthRepository.login()`/`AuthRepository.kt` disesuain (`response.token` langsung, bukan `response.data?.token`).
4. **IP LAN PC berubah** — `NetworkModule.kt` `BASE_URL` di-update ke IP baru (`10.10.100.220`, dari `192.168.18.9` lama) — dicocokin ulang tiap kali WiFi reconnect, sama kayak catatan sesi lalu.

### UI polish lain-lain
- Kartu tier Plafond di Home: `width(150.dp)` tetap → `widthIn(min = 168.dp)` (lebih lega + otomatis melebar kalau font sistem HP di-besarin via Aksesibilitas, semua Text di situ emang udah pakai `.sp`).
- 🆕 `ui/theme/Dimens.kt` — skala teks niru Tailwind (`TextSize`/`TextLineHeight`, `xs` s/d `xl9`, 1rem=16sp) sebagai referensi ke depan, belum diterapin nyapu ke semua Text() existing (nunggu diminta, biar gak jadi perubahan raksasa).
- Judul tiap layar (pola `FontStyle.Italic` + ukuran ~17-22sp) dinaikin `FontWeight.Medium`→`Bold` di Home (2x), Notifikasi, Ajukan (3x), Profil, Riwayat — biar lebih nonjol. Login/Register/Splash/Daftar Plafond ternyata udah Bold dari awal.
- Tenor picker (Home Simulasi + Ajukan Pinjaman "Durasi Peminjaman") — dari `Row(horizontalScroll)` (numpuk kiri, chip kekecilan) jadi `Row(fillMaxWidth)` + tiap `SelectableChip` dikasih `Modifier.weight(1f)` — sekarang bagi rata lebar layar.
- `RegisterScreen.kt` nabrak status bar bawaan Android — ditambah `statusBarsPadding()`.
- `EditableNominalField` (`ui/components/`, komponen baru) — field "Jumlah Pinjaman" di Home sekarang bisa diketik numpad langsung (selain slider), sinkron dua arah, format `Rp1.000.000` pakai `VisualTransformation` biar cursor gak berantakan. Baru dipasang di Home; belum di Ajukan Pinjaman (user minta scope Home dulu).

### Master Rate (Angular) — halaman admin baru buat kelola tenor/bunga
Ternyata gak ada UI buat manage `tbl_bunga_tenor` sama sekali (beda dari Plafond yang udah ada Master Plafond) — backend-nya sendiri udah lengkap (`GET/POST/PATCH/DELETE /api/v1/bunga-tenor`, superadmin-only). Dibikinin niru pola persis `master-plafond`:
- 🆕 `shared/models/bunga-tenor.model.ts`, `shared/components/bunga-tenor-form-modal/`, `pages/superadmin/master-rate/`
- Route `/admin/master-rate` + sidebar entry "Master Rate" (icon `LucidePercent`, grup MASTER DATA)
- **Menu access di-daftarin lewat UI beneran** (Master Menu → tambah entry, Master Access → centang semua CRUD buat SUPERADMIN) — bukan cuma nambah route doang, karena tanpa ini `menuAccessGuard()` bakal nge-redirect balik ke `/` (row `tbl_menu`/`tbl_role_menu` gak match). Diverifikasi live di browser (login superadmin beneran, buka halamannya, kebaca semua tenor asli dari backend).
- Tenor "36 Bulan" (data lama gak jelas asalnya dari 17 Agustus) dihapus user sendiri lewat halaman ini.

### `AuthRepository.kt` — beberapa ronde perbaikan, plus 1 file orphan dihapus
User nyoba nambahin pola "observable session" (`observeSession()`, `AuthSession`, `authLocalDataSource`) niru referensi materi kelas — beberapa kali bikin file ini (dan file baru `AuthSessionLocalDataSource.kt`) gak bisa compile karena nge-reference tipe yang gak pernah ada di project ini (beda arsitektur dari yang dipakai Sakuku, yang udah punya `TokenDataStore` buat urusan sama). **Fix final**: `AuthRepository` dapet property beneran observable — `val session: Flow<String?> = tokenDataStore.tokenFlow` (delegate ke `TokenDataStore` yang udah ada, bukan sistem session+expiry baru), `TokenDataStore` ditambahin sebagai constructor dependency. `login()` sengaja TETAP gak nyimpen token sendiri (itu tanggung jawab `LoginViewModel`, biar gak dobel-write dari 2 tempat). `AuthSessionLocalDataSource.kt` (file baru, nested-class-di-dalam-class, gak ada yang makai) dihapus.

### Testing infra — baru dari nol sesi ini
1. **`AuthRepositoryTest.kt`** (`app/src/test/`, JVM unit test, JUnit4+MockK+`Enclosed` runner) — 3 grup: `Login` (sukses/gagal), `Register` (sukses/non-2xx/exception), `Session` (3 test buat property `session` baru — emit token, emit null pas logout, urutan emisi bener). **9/9 pass, dijalanin beneran via `./gradlew testDebugUnitTest`**, bukan cuma compile-check.
2. **`LoginScreenUiTest.kt`** — awalnya user taro di `src/test/` (salah total, itu instrumented test yang butuh device/emulator, gak bisa jalan di JVM polos) + nge-reference `AuthSessionLocalDataSource`/`MainActivity` yang gak valid + nyari teks "Sign In" yang gak pernah ada di app ini (tombolnya "Masuk"). **Dipindah ke `src/androidTest/java/com/example/sakuku/ui/screens/login/`, ditulis ulang total**: nge-test `LoginScreenContent` (versi stateless `LoginScreen`, udah dipisah dari awal biar bisa di-`@Preview`) LANGSUNG — gak butuh Hilt/network/MainActivity sama sekali, jadi cepat & gak gantung backend nyala. 6 test (render, ngetik update field, tombol disable/enable pas loading, klik manggil callback, pesan error muncul). Perubahan kecil di `LoginScreen.kt` biar bisa ditest: `LoginScreenContent` `private`→`internal`, tambah `Modifier.testTag(...)` di 2 field + 1 tombol (visual/behavior gak berubah).
3. **Batasan penting**: dari sesi ini, gak ada 1 pun instrumented test (`androidTest`) yang bisa diverifikasi PASS oleh Claude Code sendiri — butuh device/emulator konek yang gak ada di environment ini. Unit test JVM (`src/test`) BISA dijalanin+diverifikasi langsung. User sempet coba run instrumented test-nya sendiri, gagal duluan di `AndroidTestDeviceInfoPlugin` (`Connection timed out: connect` ke ADB, 0 test yang sempet jalan) — itu masalah environment/emulator, bukan bug di test.

### 🚨 Gap besar ketemu: registrasi customer 100% gagal sampai SMTP diisi, dan errornya menyesatkan
Alur register sekarang wajib kirim OTP dulu (`status=PENDING_VERIFICATION`), tapi `.env` backend (`MAIL_USERNAME`/`MAIL_PASSWORD`) masih KOSONG — `EmailService.sendOtpEmail()` gagal dengan `AuthenticationFailedException` ("Authentication failed", dari Jakarta Mail nolak SMTP auth). **Bug tambahan yang bikin ini susah kedeteksi**: `GlobalExceptionHandler.java` (backend) punya catch-all `@ExceptionHandler(RuntimeException::class)` yang nge-mapping SEMUA runtime exception yang gak spesifik ke-handle jadi **HTTP 404**, jadi error SMTP nyamar sebagai "Not Found"/"Authentication failed" yang keliatannya kayak masalah auth/routing, padahal sama sekali gak ada hubungannya. **Belum di-fix sesi ini** (bukan bug kode yang perlu diperbaiki di catch-all-nya — itu emang desain lama, dicatat aja) — nunggu user isi App Password Gmail asli ke `.env` (`MAIL_USERNAME`/`MAIL_PASSWORD`), belum dieksekusi sampai akhir sesi ini karena user belum kasih kredensial final. **Konsekuensi penting**: begitu SMTP kekonfig, masih ada 1 gap lagi — **layar OTP verifikasi belum pernah dibikin di Android** (sengaja ditunda beberapa sesi lalu), jadi customer baru bisa register tapi gak ada tempat masukin kode OTP-nya sampai layar itu dibikin. Next task.

### Tier Plafond — dikonfirmasi, 1 beda disengaja
`GET /api/v1/plafond` dicek live: Bronze 5jt / **Silver 10jt** (beda dari rencana awal 15jt — user konfirmasi disengaja) / Gold 25jt / Platinum 50jt.

### File baru/berubah sesi ini
**Android (`Sakuku`)**: 🆕 `ui/screens/MainViewModel.kt`, `ui/components/EditableNominalField.kt`, `ui/theme/Dimens.kt`, `app/src/test/java/.../data/repository/AuthRepositoryTest.kt`, `app/src/androidTest/java/.../ui/screens/login/LoginScreenUiTest.kt`. ✏️ `ui/screens/MainScreen.kt`, `ui/components/AnimatedBottomNavBar.kt`, `ui/home/HomeScreen.kt`, `ui/theme/Gradients.kt` (`sakukuBlobBackground()`, sesi sebelumnya, dipasang lebih luas sesi ini), `data/repository/AuthRepository.kt`, `data/remote/ApiService.kt`, `di/NetworkModule.kt`, `navigation/AppNavigation.kt`, `ui/screens/login/LoginScreen.kt`, `ui/screens/register/RegisterScreen.kt`, `ui/screens/pengajuan/PengajuanScreen.kt`, + 7 layar lain buat blob background (Notifikasi/Riwayat/Profil/Plafond/Onboarding/Welcome). ❌ `data/local/AuthSessionLocalDataSource.kt` (orphan, dihapus).

**Angular (`angular-challenge`)**: 🆕 `shared/models/bunga-tenor.model.ts`, `shared/components/bunga-tenor-form-modal/`, `pages/superadmin/master-rate/`. ✏️ `app.routes.ts`, `shared/config/sidebar-menu.config.ts`.

**Backend (`saku-ku`)**: gak ada perubahan kode sesi ini — cuma investigasi (SMTP gap, tier plafond, hapus 1 row `tbl_bunga_tenor` lewat UI/API resmi, bukan raw SQL).
